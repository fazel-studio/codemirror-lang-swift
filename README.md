...
Create a Swift language support extension. No configuration is required for v0.1.
...
- `return`/`break`/`continue` with trailing expression/label is highlighted correctly but the trailing part is parsed as a separate statement in v0.2 due to LR(1) limitations without newline tracking (same as v0.1) — `return x` is `return` + `x` as next statement; `break outer` is `break` + `outer` as next; this does not affect highlighting but affects tree structure for those statements.
